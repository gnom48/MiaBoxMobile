...
import com.example.pronedvizapp.requests.RequestsRepository.orderCallTranscription
...
class CallsInGroupAdapter(private val context: Context, private val callsInfo: List<CallInfo>, private val callsRecords: List<CallRecord>) : BaseAdapter() {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (callsRecords.find { it.id == callsInfo[position].recordId } == null || callsRecords.find { it.id == callsInfo[position].recordId }?.name == "no file") {
            binding.transcriptionButtonTextView.visibility = View.GONE
            binding.playImageButton.visibility = View.GONE
            ...
            if (callsInfo[position].transcription != null && callsInfo[position].transcription != "" && callsInfo[position].transcription != NO_TRANSCRIPTION) {
                ...
                bindingDialog.saveButton.setOnClickListener { view ->
                    CoroutineScope(Dispatchers.IO).launch {
                        val taskResponse = orderCallTranscription(context, MainStatic.currentUser!!.id, callsInfo[position].recordId, "base", MainStatic.currentToken)
    ...
    private fun openAudioFile(file: File) { // TODO: ошибка проигрывателя - нет разрешения, иногда
    ...
    private suspend fun downloadAndPlayAudioFile(fileName: String, recordId: String) {
